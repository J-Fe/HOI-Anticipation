...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,args,is_val=False,isSegment=False,isAnticipation=True): ####### Eval 할때는 Segment Ture 로 바꿔야함
        self.is_val = is_val
        ...
        if isAnticipation:
            video_length = len(self.load_data) # load_data는 그러니까 하나의 영상에 대해서 정리된 데이터 셋 집합체임. 내가 메모장에 저장해놓은것처럼 
            load_data = []
